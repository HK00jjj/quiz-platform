...
/* ⑱-23 题干声音条（2026-09-15 傍晚）：声音控件原在解析区，答题中无任何重读入口
   ...
   2026-09-21 修断言腐化（INC-20260921-11）：旧正则锚定字面量「🔁 重读题干」，但按钮
   emoji 已改 IconReplay 组件，锚点失配后断言长期红着没被发现（上一会话未跑全套回归
   就部署）。改锚真实 DOM 顺序：起点 → toggleTts → replayTts → 重读题干文本，并加
   距离上限（600/300 字符）防跨区误配——契约本身不变：答题中可见开关+重读入口。 */
ok(/ttsOK && !answered && !showAnswer && \([\s\S]{0,600}?onClick=\{toggleTts\}[\s\S]{0,300}?onClick=\{replayTts\}[\s\S]{0,300}?重读题干/.test(practiceSrc), '⑱-23 题干声音条：答题中可见（开关+重读题干），绑定 toggleTts/replayTts（2026-09-21 锚点修复）')
