...
| **答题卡牌尺寸**（高度优先，不再用固定比例） | `.q-card-wrap`：宽 `min(94vw,620px)`，高 `min(calc(100svh - 96px), 宽/0.5)`；`.q-flipper` 用 `height:100%` 跟随 | 实测 390×844 → **366.6×733.2**（占视口高 86.9%，下方仅留 36.8px）；1280×900 → **620×804**（89.3%，留 38px）。两端 `docOverflow:false`、牌面无内部滚动。**96px = 舞台内边距(14+16)+进度行(~30)+其外边距(14)+余量，改进度行时要同步**；移动端用 `svh` 不用 `dvh`，否则地址栏收放会让卡牌反复重算尺寸 |
| 判断题横幅 | `.judge-pair` 纵向堆叠居中；`.judge-card` 宽 `min(70%,320px)`、高 `clamp(96px,24cqw,150px)`；铜牌素材（378×640）按 `auto 86%` 做左侧徽章 + `17% center` 定位，不裁不拉 | 实测手机 192.9×96、桌面 320×122.9。**不要用 `aspect-ratio:16/9`**：桌面会顶到 320×180、两张叠 370px，把答案区挤到折叠线下（sh 684 > ch 649）。另：`cqw` 按容器 **content box** 解析（牌面 548 − padding 36 = 512，所以 24cqw = 122.9 而不是 131.5） |
