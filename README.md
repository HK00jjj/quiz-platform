...
| 对象 | 实测值 | 用在哪 |
| --- | --- | --- |
| p2 牌体（已裁透明留白，从 2457×4701 原图重取） | 768×1470，比例 **0.5224** | `.q-flipper` / `.bank-item` 的 `aspect-ratio`；题库牌正面背景 |
| p2 金饰内缘（藤蔓/尖拱/龙首） | 上 29% 右 18% 下 13% 左 19.5% | `.tarot-face .face-in` 的 `inset`（题库牌） |
| **p19 龙首卷轴**（从 5120×2868 原图重取） | 1600×896；杆区 x≈5~15% 与 84~92%，焦边 y≈5~14% 与 78~88% | **答题页正面**：`border-image: 125 256 179 256 fill / 26px 52px 36px 52px stretch`（边框宽 = 切片×0.2 统一缩放 → 四角龙首不变形，杆上藤蔓仅纵向拉 19%） |
...
- 答题页正面是**一整张摊开的羊皮纸卷轴**（p19 九切片做龙首杆 + p4 无缝纹理做纸面），题目直接写在纸上；
  牌背仍是 p6 塔罗牌背——翻牌的叙事就是「塔罗牌翻过来变成一张摊开的卷轴」。
  谜面不要再套一层纸板（`.parch-layer` 已改为透明 + 一条朱砂栏线），否则又是“糊上去”。
- 答案默认被蜡封条遮住（`.seal-lock`，钉在牌底不随正文滚动）；答题后蜡封三帧碎裂，
