...
**验证**：build 主包 `index-CZzZnwlT.js` 534.32 kB；六步链全绿（deploy **`5ce363b`** 父=`65c3b62` → verify-deploy IDENTICAL 134/134 → push-src **`8e3537f`** → verify-live ALL OK 134/134 三哈希 MATCH）；**线上真机 E2E 15/15 PASS**（第三轮，ttsReq 5→3——解析预载与揭晓共享同一条请求，揭晓零新合成请求省一整轮 RTT；A9 重读 reqs=0、A8 无断链、A6 零 error）。

**E2E 口径修正（stem_read_e2e.cjs 两处，复用 harness 必读）**：预载提速后窗口内会混入解析预载请求（text 以「正确答案」开头）——①A1/A3 判定对象改为「squeeze 后以题干前缀开头的那条请求」，预载不参与（首跑 13/15 的 A1/A3 FAIL 均此误伤）；②A4 改联合判据「揭晓后有 GA start（朗读开口）+ 全量请求存在『正确答案』开头文本（预载/揭晓共享同一请求不限时序）」——揭晓命中缓存零新请求时原请求级窗口判定会误报（次跑 14/15 的 A4 即此）。另：第三轮曾一次运行 9 分钟卡死（登录/加载网络波动连锁超时），杀任务重跑即恢复 40s 全绿，属暂态。
...
**需求**（用户）："1.中途点击播报关，在重新点击播报开，从暂停位置继续读。2.全面审查，切底解决语音卡顿，停顿问题。"

**根因（卡顿/停顿，实事求是取证）**：桌面自动档此前走 native speechSynthesis，每块 ~180 字 = **独立 utterance = 独立网络合成**，自带首尾静音 + 块间切换空隙（0.3~1s）；块边界≈句号边界 → "每个句号卡一下"的主因。微软 `<audio>` 双缓冲线只消网络等待，消不掉独立文件拼接的固有空隙。

**实现（commit `63217f7`，src 备份 `8faadc0`；上线 bundle index-9KSn5BRv.js）**：
1. **GA 无缝管线（tts.js）**：`fetch` 块 MP3 → `decodeAudioData` → `gaTrimRange` 裁首尾合成静音（1% FS 阈值，纯函数可测）→ 单 `AudioContext` 时间线 `AudioBufferSourceNode.start(when, offset, duration)` **采样级拼接——块间零间隙零元素切换**；5ms 淡入淡出消接缝爆音；下一块预解码（`gaCache` 跨会话缓存+失败逐出）；单块失败跳过不挂死。仅 Edge 神经音 + webAudioOK 走 GA（百度无 CORS 仍走 `<audio>` 旧线），`speakCloudLine` 分流；`stopCloud`/`pauseCloud`/`resumeCloud`/`unlockCloudAudio`/`prefetchCloudFirst` 全部接线 GA。
...
**诚实边界**：真听感（块间无停顿、暂停续读位置精确）待用户真机验收；GA 首响=手势窗口+首块解码+合成 RTT（gaCache 命中后同文近即时）；沉浸批2 的 B3 长会话时序门控/B4 boot 每会话一次行为归其会话文档。
...
**症状**（用户）："语音读每次句号后面就会停顿一下，有点影响沉浸感。"

**根因**（实事求是取证）：`chunkSpeechText` 按句读切块（句号是首选断点）→ 块边界≈句号边界；每块是**独立合成音频，自带 ~0.1-0.3s 首尾静音**（微软神经音单请求 padding）+ "ended 派发→step→play 管线"切换空隙。双缓冲只消了网络等待，消不掉"独立文件拼接"的固有空隙——每个句号处的停顿 = 尾静音 + 切换空隙 + 头静音。

**实测边界**（probe_cloud_url.cjs，2026-09-15 中午）：微软两跳代理（Supabase tts 函数）**后端硬限单请求 ≤300 字**（450 字 → 400 `{"error":"text too long","max":300}`）；150 字热合成 ≈2.2s（冷启动 7.3s）——首块必须保持小（首响优先），这是"点开解析不立马读"的物理下限。

**修复（commit `2d823c4`，父 `0c180c9`，bundle index-sejYtndQ.js）**：
1. **块长分级**：`chunkSpeechText(raw, max, firstMax)` 新增首块独立上限——首块 ≤150（首响快），微软两跳线后续块 ≤300（`CLOUD_CHUNK_MAX_EDGE`，块边界减半=句号停顿减半）；百度备用线维持全 150。线路决策 `cloudChunkMaxFor(voice)`：`zh-*`（Edge 神经音）→300、百度 `__cloud_baidu__`→150。speak 三处云端分支与 prefetchCloudFirst **同参切块**（chunks[0] URL 逐字节一致才命中手势预载）。
2. **尾部提前续播（TAIL_LEAD=0.18s）**：speakCloud 播放位 rAF 轮询，`currentTime >= duration-0.18`（落在句尾衰减/静音区，无听感）就提前 `step()` 切播预载好的下一块——下一块开头静音正好补齐切换空隙 → 块间听感连续。兜底/降级链：`onended`（提前切失效时照常推进）→ `onerror`（单块失败跳过）→ 无 rAF 用 60ms 轮询 → 后台标签页 rAF 暂停自动回落 onended。暂停/续播/stopCloud 全部经守卫自停。

**验证**：run-all **14 套件 ALL GREEN**（tts.regression **155 断言**，⑯ 组 7 断言新增：首块≤150/后续≤300/拼接恒等/默认参兼容/两线决策）；E2E v9：音频零 error PASS、6 题 6 play 无回归、play 全部成功启动；部署链一次全过：build → purge DIST CLEAN → deploy **`2d823c4`** → verify-deploy **IDENTICAL** 147/147 → push-src **`ac2cee7`** SRC BACKUP OK → verify-live **ALL OK** 147/147 三哈希 MATCH（本轮未遇 Pages 传播延迟）。

**诚实边界**：块间停顿改善量（300 字块 + 提前续播）**待用户真机验收**；首响下限 ≈ 手势窗口 + 首块合成 RTT（热 ~2.2s、冷 ~7s，同块 24h 缓存命中则快）——若仍不满意，下一步可做：TTS 代理改 POST（摆脱 URL 长度限制→整段一次合成）或预合成缓存池。
...
**任务来源**：上一账户会话（工作区 2026-09-13-22-05-50，09:19~09:27）做「normalizeSpeech 扩电气自动化
读法词典」（词形来源=题库实测 acronym_inventory.txt 前 60），主体已落 tts.js（①-b 单位段 + TOKEN_READ 扩充
+ ⑭ 组断言），但卡在 `tts.regression` ②-2 红且被 PowerShell 控制台 mojibake 误导（怀疑正则/模块缓存）。

1. **乱码真相**：debug 脚本 console.log 中文 → PowerShell 5.1 管道按 GBK 解码 → 文件里是双重转换的假乱码；
   码点行（纯 ASCII）证明函数输出完全正确（"按SB 2，KM吸合"）。**教训：node 中文输出必须脚本内
   fs.writeFileSync(…,'utf8') 落盘再读，绝不走 console.log 管道**（与交接环境坑同源，此为加深案例）。
2. **唯一真修复**：`TOKEN_READ` 补 `KM: 'K M'`（接触器代号，清单 82 次）——断言期望"K M"逐字母而表里无词条。
   SB 不进表是既定口径（⑭-10：不在清单的词保持原样，Modbus/ON 锁定）。
3. **意外发现并修复**：`scripts/purge-dist.mjs` 第 98 行 `*/` 提前闭合块注释 → 99 行 `【2026-09-14 事故后加码】`
   成裸代码 → SyntaxError。cd683ad 部署之后本地被改坏（时点无法确证，与词典改动无关）。修法：删 98 行尾
   `*/`，95~104 合并为一段注释。**接手者注意：部署链任何一步 SyntaxError 先查脚本文件本身是否被改坏。**
4. **补记两笔漏档提交**（上一会话 cd683ad 之后所做，此前未入账）：
   `fc19afd`（手机端音色选择器重做：原生 select → 自绘底部弹层 ≈56vh 可滚动，去分组标题行）、
   `762b379`（按用户口径：音色列表/指南不再标注"与电脑端一致"——两端本就相同）。
5. **部署链（全过）**：build `index-sEy0k3Uo.js`(515.32 kB) → purge DIST CLEAN → deploy **`aaf33f4`**（121 blobs）
   → verify-deploy **IDENTICAL**（121 文件 0 缺 0 错 0 多）→ push-src **`541ae45`** SRC BACKUP OK
   （.github/ 降级跳过属预期）→ verify-live **ALL OK**（121/121 200，三哈希 MATCH，17.1s）。
6. **回归**：run-all 14 套件 ALL GREEN；tts.regression **147 断言**（131 + 16 条词典新口径，上一会话加）。
...
10. **「声音」合并控件 + 语速自定义**（用户三轮调值后拍板"不要把调速和声音分开、不要预设档位"）：
    解析区右上角**只保留一个芯片**——收起时显示当前状态（`🔊 1.35×` / `🔇 静音`），
    点开是「🔊 播报开/关 + 无级滑块 + 数值」。滑块区间 **0.5~2.0，步进 0.01**
    （真机实测 0.05 时滑块会把 1.42 吸附成 1.40，网格 = 0.5+0.05n，故收细），
    偏好存 `qp.tts.rate`（默认 `TTS_RATE = 1.35`）。拖动防抖 450ms 后用新语速重播当前题
    （用户主动触发，非自动重启）；显示走 `fmtRate()` 去浮点尾巴。
...
13. **移动端排查（次日晨，用户问"手机版为什么没有"）**：手机视口 + 三种 UA
    （安卓 Chrome / iPhone Safari / 微信安卓内核）真机模拟实测——**控件在手机上一直有**：
    `🔊 1.35×` 位于 `◇ 解析` 标题行右侧，`elementFromPoint` 遮挡检测通过、在视口内
    （412×915 下 x≈278 y≈161），截图见 `E:/workbuddy-cc/2026-09-13-22-05-50/probe_t34.png`。
    所以"没有"的真实成因是两类，都已补齐：
    · **不支持语音合成时此前静默不渲染** → 现在解析区给可见解释
      「🔇 本浏览器不支持播报」（title 写明换 Chrome/Edge/Safari），
      用户不会再以为是漏做（stub 空壳 API 实测：提示出现、控件不渲染、0 JS 错误）；
    · **能力探测收严**：`ttsSupported()` 从 `'speechSynthesis' in window` 改为
      「存在且 `speak` 是函数」——部分安卓 WebView 会挂空壳属性，旧判定会渲染出点了没反应的控件；
    · **手势解锁**：新增 `unlockSpeech()`，在「查看解析 / 展开参考答案 / 题库 🔊」的点击
      处理函数里同步播一个 0 音量空 utterance —— 浏览器（尤其 iOS Safari）要求首次 speak
      必须在用户手势栈内，而我们的自动播报发生在点击后 520ms（等蜡封动画），不加解锁会出现
      "点了没声音"。
    另：手机端还有一种解释是**缓存旧包**（控件是 09-13 23:00 后才上线的），
    已按 HANDOFF 第十二批第 10 条的办法排查（`curl` 线上包 grep 特征值 + 强刷）。
14. **音色强化（次日，用户两连报"机械音太重""Edge 上也机械"）**——抓到两个真 bug，
    都在"语音列表是异步补齐的"这个事实周围：
    · **只等 800ms 就开口**：Edge 首帧常常**只列出 3 个老 SAPI 中文音**（本机实测：
      页面载入时 `getVoices()` zh 数 = **0**，揭晓后才 17 个），于是第一段就是
      Huihui 那种电子音。`waitVoice()` 改为「等到出现**非 SAPI** 音再开口，最长 1500ms，
      超时才用当前最优」；
    · **每块各自 pickVoice**：列表中途补齐时，前几块老音、后面换成神经音 =
      用户 09-13 就报过的"读一半换音色"。改为 **整段锁定同一音色**：链开始解析一次并把
      `voiceName` 存进 session，之后每块按名字现取（对象可换、名字不换），
      `resumeSpeak` 也沿用同一 name（新增纯函数 `resolveVoiceByName`）。
    · **新增音色可自选**：面板第二行「音色」下拉（`listVoices()` 只列普通话、按
      神经音>网络音>其他>老 SAPI 排序，`voiceQualityOf()` 分档；偏好存 `qp.tts.voice`，
      `pickVoice` 优先尊重用户选择），换音色即刻重播当前题便于对比；旁挂一行
      `voiceAdvice()` 提示（自然语音已就位 / 网络音不稳定 / 本机只有老式本地音）。
      ⚠ 分类器坑：`localService===false` 不能单独用来判"网络音"——某些环境 Huihui 也是
      false，会把它误判成 network；规则是**先按名字认老 SAPI**（回归 ⑦-3b 锁住）。
    · **听感优化**：Google 网络音块长 50→**70** 字（原 50 太碎，"一顿一顿"更像机器人念短语），
      神经音维持 180，本地 SAPI 维持 50。
    **线上实测（Edge/Chrome 各一轮）**：Edge 真实播报音色 100% = `Microsoft 晓晓
    Online (Natural)`、整段唯一、单块 166 字；Chrome 100% = `Google 普通话（中国大陆）`、
    块长 48~68；Edge 面板 10 个选项（自动 + 6 个 ★自然 + 3 个 ·老式）、提示语正确。
    8/8 断言通过（`voice-final.cjs`）。测试升至 63 断言，run-all 15 套件 ALL GREEN。
    ⚠ 现实约束（回答用户"为什么机械"）：**Chrome 只有 3 个老 SAPI 中文音 + Google 网络音
...
15. **"上述功能都不见了"事故（2026-09-14 晚，当天第 8 次部署后用户报障）· 已修并验证**
    ...
    **验证**：复跑同一失败测试 → **navigations=2（自动刷新已发生）、页面正常渲染、走完
    ...
    不要先怀疑代码丢失。
16. **移动端 Edge 不能选择语音（2026-09-14 深夜，用户报"手机端 edge 不能选择语音"）**
    先定性（查微软官方双份问答 + 社区）：
    · **安卓 Edge 的 speechSynthesis 默认走手机系统 TTS 引擎**，Edge 自带的云端神经音
      （"Microsoft …(在线)"，即晓晓/云希那批）**默认不会被初始化**；用户必须先手动用一次
      Edge 的「大声朗读」，浏览器才会下载语音模型并让网页 JS 看到它们；
      **没有任何 JS API 能强制初始化**（微软官方明确：设计如此，防后台耗流量/耗电）；
    · 另有官方问答记录 Android Edge 的 `getVoices()` **直接返回空**（已装系统语音包也无用），
      仅"大声朗读"功能正常；
    · 通用坑：**部分安卓内核不派发 `voiceschanged`**（WebView/Electron 长期 issue），
      只监听该事件会让移动端永久拿到空列表。
    **我方真缺陷（本次修复）**：面板原先只靠 `voiceschanged` 刷新语音清单 → 安卓上不触发
    → 下拉永远只剩"自动"，用户看到的就是"不能选择语音"。修为**多点触发刷新**：
    挂载后 8 档短轮询（400ms~9s）+ 每次打开面板 + 换音色后 1.8s + 手动 **↻ 重新读取** 按钮；
    并在列表为空时给出**可操作**指南（`voiceGuideText()`：安卓 Edge 先用一次「大声朗读」再点 ↻；
    安卓其他浏览器去 系统设置→无障碍→文字转语音 装中文语音数据），而不是一片空白下拉。
    同时保留兜底：列表为空时播报仍走系统默认音（不报错、不静默失效）。
    **验证**（`mobile-voice-verify.cjs`，线上、手机视口 + 安卓 UA）：
    场景A 注入「前 3 秒 getVoices 为空 + 屏蔽 voiceschanged」→ **不点任何按钮，下拉自动补齐到
    6 个中文音色**（证明轮询有效）；场景B「始终为空」→ 下拉仅"自动"、指南含"大声朗读"、
    ↻ 在、**播报仍发生**（calls=11）；两场景 **0 JS 报错**；8/8 断言通过。
    线上 bundle `index-CdA09Llh.js`，verify-deploy IDENTICAL + verify-live **68/68 全 200**，
    run-all 15 套件 ALL GREEN。
17. **"播报有错别音"全面排查与修复（2026-09-15 凌晨）——先取证后动手，抓到两类真 bug**
    **取证方法**（`tts-text-audit.mjs`，只读）：把全库 2301 题的**真实送读文本**
    （`正确答案：X。解析：<四段>`）按真实切块规则跑一遍，统计高风险读法与"词被切断"。
    送读规模：均 503 字/题、最碎口径 14.4 块/题、共 33090 块。
    **抓到的真 bug（可复现，非猜测）**：
    · **切块切断词/数字/括号**：拉丁词被切两半 13 处（seq 1588 `…与LOPA（La` | `yer of Prote…`
      = Layer of Protection 被截断）；数字被切碎 27 处（seq 181 `…= 375` | `0Ω，即3.75kΩ`
      —— **3750Ω 会被念成"三七五"+"零欧姆"**）；括号跨块 6 处；
    · **内部标注泄漏进播报**：`[错因:…]`（规则 v6.9 的可选标签）**1475 处**、`【概念】` 等
      段落标签 **9318 处**原样送引擎 → 会被念成"中括号/六角括号"，既噪声又干扰语义。
    **读法风险盘点（按出现次数）**：多音字 38727、拉丁缩写 14027（PLC/CPU/LVRT）、
    圆括号 8257、数字紧贴字母 5912、单位/数学符号 4811（Ω ± ≈）、斜杠组合 2038（AC/DC、I/O）、
    希腊字母 1686（τ φ η β）、`{}` 残留 8。
    **修复（`app/src/lib/tts.js`，只改"送引擎的副本"，屏幕原文一字不动）**：
    ① 新增 `normalizeSpeech()` 读法归一化词典：`Ω→欧姆`、`kΩ/MΩ→千欧/兆欧`、`μF/μA/μs→微法/微安/微秒`、
       `℃/°C→摄氏度`、`±→正负`、`≥/≤/≠/≈→大于等于/小于等于/不等于/约等于`、`×/÷→乘/除以`、
       `√→根号`、希腊字母按工程口语译名（φ→斐、η→伊塔、Δ→德尔塔、τ→套…）、
       `AC/DC→AC 或 DC`、`kVA→千伏安`、`kvar→千乏`；
       **真实原文里的数学排版符号**：`U+2212 减号→减`、上标 `²/³→平方/立方`、
       下标 `U_F→U F`（不念"下划线"）、公式里的 `=→等于`；
    ② `cleanSpeechText()` 扩写：剥 `[错因:…]`、`【概念】→概念，`（去符号留停顿）、`{}` 只留内容；
    ③ `chunkSpeechText()` 新增 `safeCutIndex()` **切块边界保护**：不在词/数字中间切、不留下悬挂算子
       （`= （ ( + - × ÷ ·`）、右侧不留孤立右括号、断点处括号必须配平（前后各 24 字内找安全点）。
       ⚠ 踩坑记录：第一版回退搜索窗口写成 `k > cut - 14`，**差一格**导致真正的安全点恰好被排除，
       实测仍在词内切断 —— 已改为 24 并用真实题（seq 181/1588/347）做回归锁死。
    **验证**：TTS 回归 **67 → 87 断言**（新增 ⑨ 归一化 8 条、⑩ 标注剥离 3 条、
    ⑪ 边界保护 4 条（用真实题原文）、⑫ 数学排版符号 5 条），run-all 15 套件 ALL GREEN；
    线上终检（`tts-speech-verify.cjs` 连做 8 题抓真实送读块 59 个）：
    **`【】`/`[错因]`/`{}` 泄漏 0、Ω/μF/℃/±/≥/×/斜杠/下标/上标/希腊字母残留 0、切块切断 0**，
    且归一化确实生效（送读文本里出现 千瓦/乘/德尔塔/或）。
    线上 bundle `index-BvE0T3qL.js`，verify-deploy IDENTICAL + verify-live **74/74 全 200**。
    **未动（按"不盲改"原则留给你决策）**：拉丁缩写 14027 处的逐字母读法（引擎差异大、强改风险高）、
    多音字 38727 处（语境相关，需词表且应做成"仅影响播报副本"的同音替代表）、
    希腊字母译名口径是否采用工程口语版（φ=斐 等）。
18. **默认音色改为云健（2026-09-15 用户指令）**：`pickVoice()` 自动档首选顺序改为
    **云健 → 晓晓 → 云希 → 其他神经音 → Google 网络音 → 老 SAPI**；
    面板"自动"标签同步改为「自动（默认云健）」。
    ⚠ 关键语义：**用户在面板里显式选过的音色（`qp.tts.voice`）依然优先**，
    本次只改"自动"档的回落顺序——所以若想切回云健，需要在面板里选「自动（默认云健）」。
    **验证**：TTS 断言 87 → **89**（新增 ⑧-4 有云健时首选云健、⑧-5 无云健时回落晓晓），
    run-all 15 套件 ALL GREEN；线上真机（`voice-final.cjs`，清空偏好后揭晓）：
    **Edge 真实播报音色 100% = `Microsoft 云健 Online (Natural)`**，整段唯一音色；
...
用户指令「全流程全部颗粒度对齐」——把知识点统计口径在全流程统一到 **K 域族级（knowledgeDomain）**，
细粒度 kp 只保留给查重（kpNorm）/变式（stats.js byKp）/检索（Bank 搜索）。全流程分布实证：
组卷薄弱度（ability.js v4.15）、题库筛选展示、闸6 校准、pyBKT、K 谱系覆盖矩阵**早已是 K 域口径**；
validate.js 知识域枚举闸（缺字段/非 K1~K27 均 error）**09-11 已在位**——唯一错位点 = 晋级赛闸③。

1. **闸③聚合键 K 域化（Learn.jsx）**：旧实现按细粒度 kp 字符串精确聚合，KP_MIN=3 下绝大多数
   kp 样本不足被豁免 → 闸③名存实亡（K8 440 题挂 429 细分点，≈1 题 1 点）。改为 K 域聚合后
   与 pickMatched 薄弱度/闸6/覆盖矩阵同口径——闸③首次真正度量"域掌握度"（本轮每个作答
   ≥3 次的 K 域正确率须 ≥95%）。**语义收紧已向用户披露并获批准**。
2. **masteryGate 提取纯函数（ability.js）**：三闸计算原在 useMemo 内零测试覆盖，提取后可回归
   锁定；字段与旧实现一一对应，覆盖/逐题掌握闸数值行为一字未变。无域题兜底回落 knowledgePoint
   （与旧行为等价，防畸形数据制造假闸；全库 1531 题实测全带 K 域标注，纯防御分支）。
3. **回归**：新增 tests/mastery-gate.regression.mjs（15 断言：域聚合参判/达标路径/豁免语义保持/
   无域兜底/since 时间窗/覆盖中间量/空库防除零/阈值不动——①组用例即"空转实锤锁"：同域两题
   各 2 次作答，旧口径恒 true、新口径正确拒判）；validate.regression.mjs 补 4 断言（K99/中文域/
   缺域=error、K27 边界合法）。run-all **13 套件 ALL GREEN**。
4. UI 文案同步："知识点达标"→"知识域达标"、失败原因"N 个知识点正确率未达"→"N 个知识域正确率未达"。
5. 部署：六步链，commit `def3c3d`，bundle index-DnIDXPnB.js（552.85 kB）/ CSS 未变。

## 2026-09-12 凌晨增量（第八批）· 顶尖段适配（用户三问"三档够不够/完美适配吗"落实）

用户追问"成长为顶尖专家，三档难度是否太少/完美适配"——判定：**三档标签不是瓶颈**（运行时适配
本就按连续预测通过率就近匹配，标签只作结构先验），真瓶颈三处，本批修掉前两处：

1. **targetDifficulty 专家段斜率（ability.js）**：t≤0.8 维持 0.15+0.2t（新手/中段一字不变，
   t=0.8 处连续 0.31）；t>0.8 斜率加大到 0.95/单位 t，ability≥0.8 时难度封顶 **0.50**
   （通过率 50%——旧公式封顶 0.35，高手永远被钉死在舒适区）。合意困难上限语义保留。
2. **abilityOf 窗口按题去重（§2 钝化修复）**：60 条记录窗口常被 ~20 道题的三遍记录/热点题灌满。
   改为**按题去重取最近一条**（60 道不同题，每题一票、以最近证据为准）——与 empDifficulty
   "首答主导、重复不掺水"哲学同源；半衰期语义变为"20 道不同题"。热点题免疫有专测
   （20错+热点40对：旧口径≈0.756 虚假高分 → 新口径 0.0347）。
3. **measure.mjs 同口径同步**（abilityOf 去重 + tPass 分段公式）——校准闭环与线上不再漂移。
4. 测试：tests/ability-expert.regression.mjs（10 断言：斜率端点/连续性/热点免疫/最新证据/
   三遍判定流/窗口按题数）；review B 段构造器改逐条异题 id（断言值一字未变，注释载明原因）。
   run-all **11 套件 ALL GREEN**。
5. **未做（③ 综合档供给）**：属出题线——发"题目："时按覆盖矩阵配比（综合档加权+零覆盖域
   K5/K19/K21/K25/K26/K27 优先），非代码项。
6. 诚实边界：去重后指数对历史用户会有小幅位移（半衰期从"20 条记录"变"20 道题"）——
   展示性指标，无门槛依赖（段位门槛走四闸不走指数），无需迁移。
...
用户指令"全部按照建议逐项优化"，落地《系统全面审查报告_20260911.md》的 P0~P3（报告存
`E:/workbuddy-cc/2026-09-11-11-41-22/系统全面审查报告_20260911.md`）：

1. **作答离线队列（P0·防丢数据）**：旧行为是云端写失败即回滚丢弃——用户答完题记录静默消失且界面无痕迹。
   改为 append-only 队列 `qp.pending.v1`（上限 800，超限丢最旧以护 5MB 配额）：失败即入队 + 5s/20s/60s
   三轮退避重放；db.js 新增 `persistAnswerIdempotent`（插入前探 (question_id, answered_at) 防重放重复，
   卡片走 upsert 天然幂等）；reload 时 `pendingRecordsMerged()` 把未同步记录并入本地视图
   （答案不再"看起来丢了"）；window `online` 事件即时补发。
2. **seq 全局化 + 归一化去重（P0）**：`importBank` 的 existing 从作用域内 `get().questions` 改为全局
   `get().allQuestions`——原实现多书并存时 seq 按书各自编号会撞车。validate.js 新增
   `normText/normId/dropNormalizedDupes`（全角→半角 + 去空白 + 小写后取指纹，`qn_` 前缀）。
   **主键 hashId 一行未动**（保住线上既有题目 id），归一化只用于导入去重。
3. **考试错题上云（P0·防绕过）**：原先删掉 localStorage 键 `qp-exam-wrongs` 即可跳过闸④"错题消号"，
   段位可信度受损。真源迁到云端 `settings.examWrongs`（`=== undefined` 三态判定——关键：不能用 `??`，
   否则"已清空"会被旧 localStorage 值复活）；含一次性迁移 + 删旧键。bundle 里残留的 1 处 `qp-exam-wrongs`
   是迁移期兼容读取，非回退。
4. **续考存档边界修复（P1）**：`saved.round <= saved.ids.length` → `<`（原式把"已完成全部题"的存档
   误判为无效，白考一次）。
5. **EWMA 双重计入修复（P1）**：`abilityOf` 原先前 min(5,len) 条被 seed 与主循环各算一次；改为 seed 后
   从 index=seedN 起算。回归用例锁定精确值 `0.16561407620920798`。
6. **洗牌四面收敛 + 冗余清理（P2）**：新增 `src/lib/util.js` 单一来源 `shuffle/shuffledOrder`
   （Practice/Learn/stats/ability 四处本地实现删除）；删除 ability.js 的 `rankOf`/`PLACEMENT_ABILITY`
   死代码（定位考从未实现）；Practice 多选题答错补文字提示"漏选 2 项（A、C）"（**零 CSS 改动**，
   守 §37「答错时正确答案不变绿」裁定）；app/vite.config.js 新增 `spa404Fallback` 插件
   （closeBundle 自动 copy index.html→404.html）——**本项取代上文上午增量第 1 条的"每次重建必须手动 cp"
   铁律**，手工步骤被物理消除；purge-dist.mjs 增 assets 孤儿清理（只删 index-*.js/css 且带空引用
   ABORT 守卫，本轮回收 548KB 的 index-CWd5z7rD.js）。
7. **流程文档收敛（P3）**：长批题集SOP_20260907.md 顶部加口径变更声明（六闸已被 PIPELINE.md 八闸取代、
   闸4独立评审与闸5用户终闸取消、多选 2~4 字母、入库脚本 = import_batch.mjs）；全流程总览_20260909.txt
   加历史快照警告 + 阈值更正（zoneAdvice 0.80/0.75 & 0.50/0.55，非 85%/45%）+ 优先级规则
   （代码注释 > PIPELINE > 出题规则 > 文档）。

### 备份链路缺陷（本次部署时暴露，已修）

**① `.env` 一直被备份上公开分支（安全）**：push-src.mjs 的 walk 只排除 node_modules/dist/.git，
`app/.env` 因此进入 src 分支（仓库 public，远端可见，182B）。已加 SKIP_RE 排除 `.env`/`.env.*`，
本次推送后 `.env` 已从分支 HEAD 移除。**但历史提交仍留有该 blob**——彻底清除需重写 src 分支历史
（删除并重建分支），属破坏性远端操作，**待用户决定**；同时建议轮换 .env 内的 E2E 口令。

**② `.github/workflows/*` 导致整批推送 404**：GitHub 要求 token 具备 `workflow` scope 才可写该路径，
本 PAT 仅 `repo` scope → `POST /git/trees` 单条命中即返回 404（实测：仅 `.github/workflows/ci.yml`
一条失败，去掉后其余全过；二分法定位）。已加 SKIP_RE 排除并**打印跳过清单**（不静默丢文件）。
若需备份 CI 配置，须给 PAT 补 workflow scope。

**③ 顺带修 push-src 潜伏 bug**：`req()` 里"4xx 不重试"的 throw 写在 try 内、被自身 catch 吞掉，
404 这类确定性错误也白跑 3 轮退避；改为先记 fatal、出 try 再抛（真实错误不再被 [retry] 噪音淹没）。
另修正 commit message 的陈旧主题名（塔罗「奥术典籍馆」→ 糖果「糖果题库」）。

**回归**：新增 `app/tests/review-20260911.regression.mjs`（21 断言：归一化去重 A 组、EWMA 精确值 B 组、
洗牌语义 C 组，含 hashId 不变性断言），并入 run-all.mjs；全量套件 ALL GREEN。

**部署**：六步链全过——build ✓（bundle index-DdMJaUAG.js 563489B / index-DEnYBEGr.css 128336B；
404.html 由插件自动生成 603B）→ purge-dist ✓（回收 548KB 孤儿）→ deploy commit `a3c5e77` →
verify-deploy ✓（Pages built）→ push-src 67/67 `RESULT: SRC BACKUP OK`（commit 4e94bc0）→
verify-live 27/27 200 OK + 三哈希 MATCH + `LIVE RESULT: ALL OK`。
特征串命中：漏选 1 / qp.pending.v1 1 / examWrongs 8 / 补传 5；死代码 PLACEMENT_ABILITY 0 / rankOf 0。

**两条自纠（诚实记录）**：① 审查阶段误报"多选答案 2~4 字母是该拦未拦"——实为 SOP 文档滞后，
...
用户批准"去除包 + 闸6 回流"两批次。**去除包（validate.js v4.14）**：21 题生成批整批通道（checkQuota/checkLayerComposition/checkDataStimulus/diffBand/序号语义豁免，约 150 行 UI 不可达死代码）与 PROTOCOL_VERSION 死常量整体移除；`validateItems(items)` 第二参占位保留（旧 Node 脚本多传实参被忽略，precheck_batch.mjs 等不受影响）；`classifyImport` 收敛为 backup/parse-error/items 三分支（store.js 仅 re-export，兼容）。`fetch_level.mjs`、`precheck_21题_20260907.mjs`、`Import.jsx.bak-20260907` 移入 `Documents/Qoder/命题流水线/_回收站/退役文件_20260909去除包/`。回归：app 17/17 + 流水线 10/10（用例②改断言"整批通道退役后不再豁免"）+ ability 23/23。

**闸6 第二轮回流（20:33，回流链复跑）**：334 条作答 / 1258 题；首答正确率 基础 69%(n=143) / 应用 63%(n=38) / 综合 88%(n=8)——倒挂实锤。**PRIOR_P 回写为 基础 0.70 / 应用 0.63 / 综合 0.75**（综合向全库客观首答率 69% 收缩，不为 n=8 全信）；"标签带随能力上移"组卷语义随之作废（P1/P1b 单测改写为倒挂校准后的诚实语义：低能力命中实测最易的综合带）。可疑题 54 题进台账复核区（明细见 `E:/workbuddy-cc/2026-09-08-21-39-09/analysis_result_20260908.json` + 20:33 版 HTML 报告）。极化：n≥2 全对 48 / ≤49% 7 / 中段 15。kp 密度：n≥3 的 kp 仅 35/177（薄弱度优先激活率低的根因）。

六步链全过（IDENTICAL、SRC OK、verify-live ALL OK 三哈希 MATCH）；bundle 特征：`基础:.7,应用:.63,综合:.75` 在位、checkQuota/数据刺激字符串 0 命中。待办遗留：组卷 kp 粒度修复（G2）、防突击闸（G3，用户未批）、难度标签按数据回炉（54 题复核）。

## 2026-09-09 傍晚增量（commit 2802ed9，bundle index-VxNj7v1R.js）· 命题侧自适应整体退役（规则 v6.3）+ 晋级赛末题计分修复

用户指令"自适应帮我去掉，AI 还是根据我给的源题进行生成"。**命题端不再读任何作答数据、不做任何升降档**——每道源题按自身难度与认知层级直接解答输出：

1. **validate.js 升 v4.13**：v4.12 verdict 复算闸（「源题难度/适配决策」三层一致性核对，checkAdaptive + verdictOf + statics 约 100 行）整体移除；`validateItems(items, batchMode)` 第三参 ctx 废弃（多传实参被忽略，历史 Node 预检脚本无需改动）。回归 17/17（原 9 个 gate 用例退役，新增 1 个"字段不再被校验"用例）。
2. **Import.jsx**：复算闸 ctx 传参移除；"📋 复制水平声明"按钮与 copyTier/tierCopied 删除；能力档条降为纯展示（文案改"题目难度完全由你提供的源题决定，AI 不做升降档适配"）；新手档难度分布提示改中性文案（不再引导补"我的水平"声明）。
3. **Learn.jsx 修复晋级赛末题计分 off-by-one**：submit() 已 setWins(+1)，nextRound 再按 verdict +1 → 交卷 pass 判定与续考存档各虚高 1 分（有效及格线变 89）。修复为 `nextWins = wins`（真实累计分）。旧续考存档若已虚高会延续至本场，新存档不再虚高。
4. **规则文档升 v6.3**（主副本 + skill 加载副本 md5 `fefa5138…` 一致）：第 0 步/纪律 3/4/4.1/4.4/第八章输入协议/字段规范/附录A 五问/Q10 全部改写为退役口径；第八章晋级的四闸描述与代码对齐（95% 线、3 次补考、错题重练）。SKILL.md 同步。fetch_level.mjs 保留磁盘但流程不再调用。
5. 网站端 pickMatched（智能匹配练习）**保留**——它是练习组卷功能，不属于命题端自适应。

六步链全过（build ✓ → purge 0 孤儿 → deploy commit `2802ed9` → verify-deploy IDENTICAL → push-src 84/84 → verify-live ALL OK 27/27 + 三哈希 MATCH）；bundle 特征串：旧文案（复制水平声明/换挡合规未做机器核对/禁止降阶/层级适配降阶）全部 0 命中，新文案（不做升降档适配/题目难度由源题决定）在位。
